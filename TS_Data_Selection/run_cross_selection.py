...
# Add current directory to path to ensure project modules can be imported
# 将当前目录加入路径，确保能导入项目模块
...
# Import all Dataset classes / 导入所有 Dataset 类
from data_provider.data_loader import Dataset_ETT_hour, Dataset_ETT_minute, Dataset_Custom

def get_dataset_class(data_name):
    """Return the corresponding Dataset class based on the name / 根据名称返回对应的 Dataset 类"""
    if data_name == 'custom': 
...
def main(args):
    ...
    # =======================================================
    # 1. Load Source Dataset (Large-scale data to be selected, candidate pool)
    ...
    source_dataset = Source_Data(
        ...
        flag='train', # Load all training set as candidates / 加载全部训练集作为候选
        size=[args.seq_len, args.label_len, args.pred_len],
        features=args.features,
        target=args.target
    )
    # shuffle=False to keep indices corresponding / shuffle=False 以保持索引对应
    source_loader = DataLoader(source_dataset, batch_size=args.batch_size, shuffle=False)
    
    # =======================================================
    # 2. Load Target Dataset (Selection Standard/Compass)
    ...
    target_dataset_full = Target_Data(
        ...
        flag='train', # Also use training set mode / 同样使用训练集模式
        size=[args.seq_len, args.label_len, args.pred_len],
        features=args.features,
        target=args.target
    )
    
    # Take a small portion of Target data (controlled by target_ratio, default 10%)
    # 截取 Target 数据的一小部分 (由 target_ratio 控制，默认 10%)
    subset_len = int(len(target_dataset_full) * args.target_ratio)
    # Ensure at least a few samples / 确保至少有几个样本
    subset_len = max(subset_len, 10) 
    indices = list(range(subset_len))
    
    target_subset = Subset(target_dataset_full, indices)
    
    # shuffle=True for stochastic gradient descent / shuffle=True 用于随机梯度下降
    ...
    # =======================================================
    # 3. Initialize Proxy Model (Linear Proxy)
    # 3. 初始化代理模型 (Linear Proxy)
    # =======================================================
    # Try to automatically infer input channels / 尝试自动推断输入通道数
    try:
        sample_x, _, _, _ = source_dataset[0]
        enc_in = sample_x.shape[1]
    except:
        enc_in = 7 # Default value if inference fails / 默认值，如果推断失败
    ...
    # =======================================================
    # 4. Run Forward-INF Cross-Domain Selection
    # 4. 运行 Forward-INF 跨域筛选
    # =======================================================
    # Initialize selector, must pass pred_len / 初始化筛选器，务必传入 pred_len
    selector = ForwardINFSelector(model, device, pred_len=args.pred_len)
    
    # Execute selection / 执行筛选
    keep_indices, scores = selector.select_data(
        source_loader,   # Candidate data / 候选数据
        target_loader,   # Guidance data / 指导数据
        lr_adapt=args.lr_adapt, 
        adaptation_steps=args.adapt_steps,
        selection_mode=args.selection_mode, # Adaptive mode: positive, gmm, top_k / 自适应模式: positive, gmm, top_k
        keep_ratio=args.keep_ratio          # Only effective in top_k mode / 仅在 top_k 模式下生效
    )
    ...
    # Convert to int64 to prevent errors when reading with PyTorch DataLoader
...
if __name__ == '__main__':
    ...
    # Source Dataset (Source domain data to be selected) / Source Dataset (被筛选的源域数据)
    ...
    # Target Dataset (Target domain data used for guidance) / Target Dataset (用于指导的目标域数据)
    ...
    # Adaptive selection parameters / 自适应筛选参数
