...
from sklearn.mixture import GaussianMixture # [New] For adaptive clustering / [新增] 用于自适应聚类
...
class ForwardINFSelector:
    ...
    def select_data(self, train_loader, val_loader, lr_adapt=0.001, adaptation_steps=1, 
        ...
        """
        Args:
            selection_mode (str): Selection strategy / 筛选策略
                - 'positive': Adaptive, keep all samples with influence score > 0 (Recommended) / 自适应，保留所有影响力分数 > 0 的样本 (推荐)
                - 'gmm': Adaptive, use Gaussian Mixture Model for automatic clustering, keep high-score cluster / 自适应，使用高斯混合模型自动聚类，保留高分簇
                - 'top_k': Fixed ratio, use keep_ratio parameter / 固定比例，使用 keep_ratio 参数
            keep_ratio (float): Only effective when selection_mode='top_k' / 仅在 selection_mode='top_k' 时生效
        """
        ...
        for _ in range(adaptation_steps):
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(val_loader):
                ...
                batch_y = batch_y[:, -self.pred_len:, :] # Slicing / 切片
        ...
        # ==========================================
        # [Core Modification] Adaptive Selection Logic
        # [核心修改] 自适应筛选逻辑
        # ==========================================
        indices = np.arange(len(scores))
        
        if selection_mode == 'positive':
            # Strategy 1: Positive Threshold (Keep all beneficial samples)
            # Set a small threshold 1e-6 to prevent floating point errors
            # 策略1: 正向阈值 (保留所有有益样本)
            # 设定一个小阈值 1e-6 防止浮点误差
            ...
            # Strategy 2: GMM Clustering (Automatically find good/bad boundary)
            # 策略2: GMM 聚类 (自动寻找好坏分界线)
            print("[Adaptive-GMM] Fitting Gaussian Mixture Model...")
            reshaped_scores = scores.reshape(-1, 1)
            try:
                # Fit two Gaussian distributions: one for noise/irrelevant, one for effective data
                # 拟合两个高斯分布：一个代表噪声/无关，一个代表有效数据
                gmm = GaussianMixture(n_components=2, random_state=42)
                labels = gmm.fit_predict(reshaped_scores)
                means = gmm.means_.flatten()
                
                # Find the cluster with larger mean (represents high Score, i.e., more beneficial)
    ...
    def _train_proxy(self, loader, epochs=1):
        ...
        for epoch in range(epochs):
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(loader):
                batch_x = batch_x.float().to(self.device)
                batch_y = batch_y.float().to(self.device)
                batch_y = batch_y[:, -self.pred_len:, :] # Slice correction / 切片修正
